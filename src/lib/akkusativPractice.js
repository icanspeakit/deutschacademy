// Akkusativ practice workspace: Fokus-Modus (one question at a time) and
...
export function mountAkkusativPractice(root, allQuestions, { onSessionUpdate } = {}) {
  const byDifficulty = Object.fromEntries(
    DIFFICULTIES.map((d) => [d, allQuestions.filter((q) => q.difficulty === d)])
  );

  const state = {
    difficulty: "leicht",
    mode: "fokus",
    focusIndex: Object.fromEntries(DIFFICULTIES.map((d) => [d, 0])),
  ...
  function currentSet() {
    return byDifficulty[state.difficulty];
  }
  ...
  function updateSession() {
    ...
    onSessionUpdate({ answered, correct, hints, total: set.length, difficulty: state.difficulty });
  }
  ...
  function renderFocusShell() {
    const set = currentSet();
    const idx = state.focusIndex[state.difficulty];
    ...
    if (idx >= set.length) {
      ...
      return `
        <div class="akk-focus-done akk-anim-in">
          ...
          <p>richtig in „${state.difficulty}“</p>
  ...
  function goToFocusIndex(nextIdx) {
    state.focusIndex[state.difficulty] = nextIdx;
    render();
  }
  ...
  function wireFocus() {
    ...
    if (restart) {
      restart.addEventListener("click", () => {
        ...
        state.focusIndex[state.difficulty] = 0;
        render();
      });
      return;
    }
    ...
    if (weiter) {
      weiter.addEventListener("click", () => goToFocusIndex(state.focusIndex[state.difficulty] + 1));
    }
    ...
    pills.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (ok) {
          ...
          updateSession();
          setTimeout(() => goToFocusIndex(state.focusIndex[state.difficulty] + 1), CORRECT_ADVANCE_DELAY_MS);
        } else {
          p.wrongCount++;
      ...
      <div class="akk-test-list">
        ${set
          .map((q, i) => {
            ...
            return `
            <div class="akk-test-row" data-qid="${q.id}">
        ...
        btn.addEventListener("click", () => {
          ...
          if (ok) {
            ...
            if (p.wrongCount >= WRONG_ATTEMPTS_BEFORE_REVEAL) {
              ...
              btn.classList.add("akk-pill--incorrect");
              updateSession();
              setTimeout(() => {
                pills.forEach((b) => {
                  b.disabled = false;
                  b.classList.remove("akk-pill--incorrect");
                });
              }, WRONG_FLASH_DELAY_MS);
            }
          }
