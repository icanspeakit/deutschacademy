...
/**
 * The same history for another trainer under its own storage key — the Präpositionen
 * trainer keeps one of these (createVerlauf("da-praep-verlauf-v1")). The exports below are
 * the Artikel trainer's instance, unchanged.
 */
export function createVerlauf(KEY) {
  let cache = null;
  ...
  const seen = (id) => Boolean(load().n[id]);

  function isSicher(id) {
    const r = load().n[id]?.r ?? [];
  ...
  function buildFehlerSession(pool, size) {
    return shuffle(pool.filter((q) => isFehler(q.id))).slice(0, size);
  }

  return { record, seen, isSicher, isFehler, stats, buildSession, buildFehlerSession };
}

const artikel = createVerlauf(KEY);
export const { record, seen, isSicher, isFehler, stats, buildSession, buildFehlerSession } = artikel;
